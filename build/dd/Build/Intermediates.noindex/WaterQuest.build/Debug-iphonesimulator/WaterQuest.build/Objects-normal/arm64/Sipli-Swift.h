...
#ifndef SIPLI_SWIFT_H
#define SIPLI_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="Sipli",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
@class CLLocation;
SWIFT_CLASS("_TtC5Sipli15LocationManager")
...
/// <code>UNUserNotificationCenterDelegate</code> singleton. Installed once in
/// <code>WaterQuestApp</code> at launch. Holds a weak <code>HydrationStore</code> so
/// action taps can log intake silently from the lock screen.
/// The singleton pattern is intentional: the <code>UNUserNotificationCenter</code>
/// delegate API is a singleton on the iOS side too, and the handler needs
/// to exist before any <code>@StateObject</code> is initialized so cold launches
/// triggered by a notification tap don’t drop the tap on the floor.
SWIFT_CLASS("_TtC5Sipli19NotificationHandler")
...
/// Default behavior suppresses the banner; return <code>.banner</code> so the
...
/// • <code>PersistedStateBlob</code>    — full authoritative PersistedState
/// Incoming (Watch → iPhone):
/// • <code>NewWatchEntry</code>         — a new HydrationEntry to add
/// • <code>DeletedWatchEntryID</code>   — UUID string of an entry to delete
/// • <code>SyncRequest</code>           — Watch asking for a full state push
SWIFT_CLASS("_TtC5Sipli19PhoneSessionManager")
@interface PhoneSessionManager : NSObject <WCSessionDelegate>
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
